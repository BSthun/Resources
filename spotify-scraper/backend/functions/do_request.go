...
func DoRequest(c *fiber.Ctx, method string, url string, body io.Reader, modifier func(r *http.Request), data any) *response.ErrorInstance {
	...
	if resp.StatusCode != 200 {
		...
		if err := json.Unmarshal(respBody, &errResp); err != nil {
			var errResp payload.SpotifyError
			if err := json.Unmarshal(respBody, &errResp); err != nil {
				return response.Error(c, true, "Unable to parse error response body", err)
			} else {
				return response.Error(c, true, "Spotify Error: "+*errResp.Error, nil)
			}
		} else {
