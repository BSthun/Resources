...
ADD https://raw.githubusercontent.com/nginxinc/docker-nginx-amplify/master/conf.d/stub_status.conf /etc/nginx/conf.d
...
ADD https://raw.githubusercontent.com/nginxinc/docker-nginx-amplify/master/entrypoint.sh /entrypoint.sh
...
RUN unlink /var/log/nginx/access.log \
	...
	&& sed -i "s/*.conf;/*.http.conf;/" /etc/nginx/nginx.conf \
	&& echo "stream { include /etc/nginx/conf.d/*.stream.conf; }" >> /etc/nginx/nginx.conf \
